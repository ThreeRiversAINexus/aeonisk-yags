...
class PromptLoader:
    ...
    def compose_sections(
        ...
        sections = prompt_data.get("sections", {})
        specialized_prompts = prompt_data.get("specialized_prompts", {})
        conditional_sections = prompt_data.get("conditional_sections", {})

        # Compose sections (check sections, specialized_prompts, and conditional_sections)
        content_parts = []
        for section_name in section_names:
            ...
            elif section_name in conditional_sections:
                content_parts.append(conditional_sections[section_name])
            else:
                logger.warning(
                    f"Section '{section_name}' not found in {agent_type} prompt (checked sections, specialized_prompts, and conditional_sections), skipping"
                )
                continue
