                        ...
                        if info:
                            ...
                            if info['type'] == 'player' and 'agent_id' in info:
                                ...
                                if player_agent and hasattr(player_agent, 'health'):
                                    ...
                                    combatant_lines.append(f"  - [{tid}] {info['name']} ({pronouns}, {faction}, {health_text}{wounds_text}{hidden_marker})")
                                else:
                                    combatant_lines.append(f"  - [{tid}] {info['name']} ({pronouns}, {faction}, player{hidden_marker})")
                            elif info['type'] == 'npc':
                                ...
                                if self.shared_state and self.shared_state.npc_agents:
                                    for npc in self.shared_state.npc_agents:
                                        if hasattr(npc, 'agent_id') and npc.agent_id == info.get('agent_id'):
                                            ...
                                            break
                                combatant_lines.append(f"  - [{tid}] {info['name']} ({pronouns}, {faction}, npc, {disposition}{hidden_marker})")
                            else:
                                # Format for enemies: [tgt_xxxx] Name (faction, enemy)
                                combatant_lines.append(f"  - [{tid}] {info['name']} ({pronouns}, {faction}, {info['type']}{hidden_marker})")
