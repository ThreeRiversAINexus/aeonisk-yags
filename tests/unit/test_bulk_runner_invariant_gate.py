...
def _dirty_events():
    # A "subdued" prisoner spawned armed = prisoner_armed ERROR.
    return [
        {"event_type": "session_start"},
        {"event_type": "enemy_spawn", "round": 0, "enemy_id": "g1",
         "enemy_name": "Independent Subdued Operative #1",
         "stats": {"health": 30, "weapons": [{"name": "Pistol"}, {"name": "Baton"}]}},
        {"event_type": "session_end"},
    ]
...
def test_dirty_session_is_flagged_with_sidecar(tmp_path):
    ...
    assert errors, "gate must report the prisoner_armed contradiction"
    assert {e["invariant"] for e in errors} == {"prisoner_armed"}
    assert all(e["severity"] == "error" for e in errors)

    sidecar = tmp_path / "invariant_violations.json"
    assert sidecar.exists(), "a quarantined run must be self-describing"
    assert json.loads(sidecar.read_text())[0]["invariant"] == "prisoner_armed"
